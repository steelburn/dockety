...
const SortableHeader: React.FC<{
  sortKey: keyof Volume;
  title: string;
  requestSort: (key: keyof Volume) => void;
  sortConfig: { key: keyof Volume | null; direction: 'ascending' | 'descending' };
...
export const VolumesView: React.FC<VolumesViewProps> = ({ host }) => {
  const [volumes, setVolumes] = useState<Volume[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [processing, setProcessing] = useState<Record<string, boolean>>({});
  const [expandedVolume, setExpandedVolume] = useState<string | null>(null);
  const { items, requestSort, sortConfig, searchTerm, handleSearchChange } = useTable(volumes, ['name', 'driver'], 'name');
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await dockerService.getVolumes(host.id);
      setVolumes(data);
    } catch (e) {
  ...
  const handleRemove = async (volumeName: string) => {
    setProcessing(prev => ({...prev, [volumeName]: true}));
    try {
        await dockerService.removeVolume(host.id, volumeName);
        await fetchData();
    } catch (e) {
        console.error('Failed to remove volume', e);
        setError('Failed to remove volume.');
    } finally {
        setProcessing(prev => ({...prev, [volumeName]: false}));
    }
  };
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Volumes</h1>
        <input
            type="text"
        ...
        <table className="min-w-full text-sm text-left text-gray-600 dark:text-gray-300">
          <thead className="bg-gray-50 dark:bg-gray-700/50">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 text-xs text-gray-500 dark:text-gray-400 uppercase">Mountpoint</th>
                ...
                <tr className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/40">
                  <td className="px-2 py-4">
                      {volume.containers.length > 0 && (
                          <button onClick={() => setExpandedVolume(expandedVolume === volume.name ? null : volume.name)} className="p-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600">
                             <ChevronDownIcon />
                          </button>
                      )}
                  </td>
                  <td className="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                    {volume.name}
                    {volume.containers.length === 0 && (
                        <span className="ml-2 text-xs bg-gray-200 dark:bg-gray-600/50 text-gray-500 dark:text-gray-400 px-2 py-0.5 rounded-full">Unused</span>
                    )}
                  </td>
                  <td className="px-6 py-4">{volume.driver}</td>
                  <td className="px-6 py-4 font-mono text-xs">{volume.mountpoint}</td>
                  <td className="px-6 py-4 text-right">
                    <ActionButton title="Remove Volume" onClick={() => handleRemove(volume.name)} className="bg-red-100 hover:bg-red-200 text-red-800 dark:bg-red-500/20 dark:hover:bg-red-500/40 dark:text-red-400" disabled={processing[volume.name] || volume.containers.length > 0}><Trash2Icon /></ActionButton>
                  </td>
                </tr>
                 {expandedVolume === volume.name && (
                     <tr className="bg-gray-100/50 dark:bg-gray-700/20">
                         <td colSpan={5} className="p-4">
                             <div className="pl-12">
                                <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Containers using this volume:</h4>
